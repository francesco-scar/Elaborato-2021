...
long move_stepper (long n_steps, float speed, float acceleration, int dir_pin, int step_pin, int enable_pin, int endstop) {      // speed in steps/s, acceleration in steps/s^2
  ...
  for (microsteps = 0; microsteps < abs(n_steps)*MICROSTEPPING_RATIO; microsteps++){
    do {
      t = micros();
      if (constant_phase) {
    ...
    if (!digitalRead(endstop)) {
      ...
      delayMicroseconds(2);      // Impulso di almeno 1us (da datasheet)
      digitalWrite(step_pin, LOW);
      delayMicroseconds(1);      // Impulso di almeno 1us (da datasheet)
      if (constant_phase) {
  ...
  for (int8_t y = 0; y < 16; y++) {
    for (int8_t x = 0; x < 16; x++) {
      gradient_modules[y][x] = sqrt(pow(matrix[y + 2][x + 1] - matrix[y][x + 1], 2) + pow(matrix[y + 1][x + 2] - matrix[y + 1][x], 2)) + 0.5;
        ...
        \vspace{1cm}
        Calcoliamo quindi le derivate parziali di $ f(x, y, r) $, espressa dell'equazione \ref{error_function}, rispetto alle tre variabili (non essendovi uno standard univoco, di seguito si è scelto di utilizzare la notazione di Leibniz per indicare derivate e derivate parziali)
